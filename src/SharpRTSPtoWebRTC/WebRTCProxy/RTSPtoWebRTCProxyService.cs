...
namespace SharpRTSPtoWebRTC.WebRTCProxy
{
	public class RTSPtoWebRTCProxyService
	{
		...
		public async Task<RTCSessionDescriptionInit> GetOfferAsync(string id, string url, string userName = null, string password = null, SIPSorcery.Sys.PortRange portRange = null)
		{
			...
			var proxy = await GetOrCreateClientAsync(_loggerFactory, url, userName, password);
